...
class Visualizer():
    ...
    def save_binaryFile(self, webpage, disp_map, image_path):

        disp_map = disp_map.cpu().float().numpy()
        disp_map = np.transpose(disp_map, (1, 2, 0))

        short_path = ntpath.basename(image_path[0])
        name = os.path.splitext(short_path)[0]
        label = 'displacement_map'
        file_name = '%s_%s.dat' % (name, label)

        image_dir = webpage.get_image_dir()
        save_path = os.path.join(image_dir, file_name)
        disp_map = np.reshape(disp_map, (256*256*3))

        disp_map = disp_map * 0.5 + 0.5

        output_file = open(save_path, 'wb')
        float_array = array('f', disp_map)
